...
private
  ...
  ... | no w≢acc =
    let acc<w = ≤∧≢→<'' acc≤w (λ p → w≢acc (sym p))
        step1 = findDiagonal-continue fuel k acc (k≮ᵇ'triangular-suc-acc k w acc acc<w Tw≤k)
  ...
  cantorPair-unpair k =
    ...
    findDiagonal-Tw≤k-aux fuel k acc Tacc≤k with k <ᵇ' triangular (suc acc) | inspect (k <ᵇ'_) (triangular (suc acc))
