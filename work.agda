...
-- Closed propositions are closed under finite conjunction (binary case)
...
closedAnd P Q (α , P→∀α , ∀α→P) (β , Q→∀β , ∀β→Q) = γ , forward , backward
  where
  -- Interleave: γ (2k) = α k, γ (2k+1) = β k
  γ : binarySequence
  γ n = if isEvenB n then α (half n) else β (half n)

  forward : ⟨ P ⟩ × ⟨ Q ⟩ → (n : ℕ) → γ n ≡ false
  forward (p , q) n with isEvenB n =B true
  ... | yes even-n = subst (λ x → (if x then α (half n) else β (half n)) ≡ false) (sym even-n) (P→∀α p (half n))
  ... | no odd-n = subst (λ x → (if x then α (half n) else β (half n)) ≡ false) (sym (¬true→false (isEvenB n) odd-n)) (Q→∀β q (half n))

  backward : ((n : ℕ) → γ n ≡ false) → ⟨ P ⟩ × ⟨ Q ⟩
  backward all-false = ∀α→P α-false , ∀β→Q β-false
    where
    α-false : (k : ℕ) → α k ≡ false
    α-false k = subst (λ x → (if isEvenB (2 ·ℕ k) then α x else β x) ≡ false) (half-2k k)
                (subst (λ x → (if x then α (half (2 ·ℕ k)) else β (half (2 ·ℕ k))) ≡ false) (sym (isEvenB-2k k))
                (all-false (2 ·ℕ k)))

    β-false : (k : ℕ) → β k ≡ false
    β-false k = subst (λ x → (if isEvenB (suc (2 ·ℕ k)) then α x else β x) ≡ false) (half-2k+1 k)
                (subst (λ x → (if x then α (half (suc (2 ·ℕ k))) else β (half (suc (2 ·ℕ k)))) ≡ false) (sym (isEvenB-2k+1 k))
                (all-false (suc (2 ·ℕ k))))

-- Open propositions are closed under finite disjunction (binary case)
openOr : (P Q : hProp ℓ-zero) → isOpenProp P → isOpenProp Q
       → isOpenProp (∥ ⟨ P ⟩ ⊎ ⟨ Q ⟩ ∥₁ , squash₁)
openOr P Q (α , P→∃α , ∃α→P) (β , Q→∃β , ∃β→Q) = γ , forward , backward
  where
  -- Interleave: γ (2k) = α k, γ (2k+1) = β k
  γ : binarySequence
  γ n = if isEvenB n then α (half n) else β (half n)
  ...
  forward = PT.rec (isSetΣ isSetℕ (λ n → isProp→isSet (isSetBool (γ n) true))) helper
    where
    helper : ⟨ P ⟩ ⊎ ⟨ Q ⟩ → Σ[ n ∈ ℕ ] γ n ≡ true
    helper (inl p) with P→∃α p
    ... | (k , αk=t) = 2 ·ℕ k , goal
      where
      goal : γ (2 ·ℕ k) ≡ true
      goal = subst (λ x → (if isEvenB (2 ·ℕ k) then α x else β x) ≡ true) (sym (half-2k k))
             (subst (λ x → (if x then α k else β (half (2 ·ℕ k))) ≡ true) (sym (isEvenB-2k k)) αk=t)
    helper (inr q) with Q→∃β q
    ... | (k , βk=t) = suc (2 ·ℕ k) , goal
      where
      goal : γ (suc (2 ·ℕ k)) ≡ true
      goal = subst (λ x → (if isEvenB (suc (2 ·ℕ k)) then α x else β x) ≡ true) (sym (half-2k+1 k))
             (subst (λ x → (if x then α (half (suc (2 ·ℕ k))) else β k) ≡ true) (sym (isEvenB-2k+1 k)) βk=t)

  backward : Σ[ n ∈ ℕ ] γ n ≡ true → ∥ ⟨ P ⟩ ⊎ ⟨ Q ⟩ ∥₁
  backward (n , γn=t) with isEvenB n =B true
  ... | yes even-n = ∣ inl (∃α→P (half n , subst (λ x → (if x then α (half n) else β (half n)) ≡ true) even-n γn=t)) ∣₁
  ... | no odd-n = ∣ inr (∃β→Q (half n , subst (λ x → (if x then α (half n) else β (half n)) ≡ true) (¬true→false (isEvenB n) odd-n) γn=t)) ∣₁
...
ι n = α , atMostOnce
  ...
  atMostOnce m k αm=t αk=t with m ≟ n | k ≟ n
